...
object maps {
  ...
  def sum[K, V <: AnyRef](ms: IArray[Map[K, V]]): Map[K, IArray[V]] = {
    ...
    ms.foreach(_.foreach { case (k, v) =>
      ret.update(k, ret.get(k).fold(IArray(v))(_ :+ v))
    })
    ret.toMap
  }
  ...
  def combine[K, V <: AnyRef](
      ms: IArray[Map[K, IArray[V]]]
  ): Map[K, IArray[V]] = {
    val ret = mutable.Map.empty[K, IArray[V]]
    ms.foreach(_.foreach { case (k, v) =>
      ret.update(k, ret.get(k).fold(v)(v ++ _))
    })
    ret.toMap
  }

  @inline final implicit class FromMapValuesOps[K, V <: AnyRef](
      private val map: scala.collection.Map[K, V]
  ) extends AnyVal {
  ...
  @inline final implicit class FromMapOps[K, V](
      private val map: scala.collection.Map[K, V]
  ) extends AnyVal {}

  @inline final implicit class MapOps[M[k, v] <: scala.Iterable[(k, v)], K, V](
      private val m: M[K, V]
  ) extends AnyVal {
    ...
    @inline def mapToIArray[A <: AnyRef](
        f: ((K, V)) => A,
        keep: ((K, V)) => Boolean = null
    ): IArray[A] = {
    ...
    @inline def flatMapToIArray[A <: AnyRef](
        f: ((K, V)) => IArray[A],
        keep: ((K, V)) => Boolean = null
    ): IArray[A] = {
