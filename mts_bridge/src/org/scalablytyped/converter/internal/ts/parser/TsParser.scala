...
/** ** We cache results of some parsers because scala parser combinators are
  * sooo slow. This means that a given `TsParser` is only valid for the given
  * `contentLength`, if provided
  *
  * @param contentLength
  *   length of content for caching optimization
  */
class TsParser(contentLength: Option[Int])
    extends StdTokenParsers
    with ParserHelpers
    with ImplicitConversions1 {
  self =>
    ...
    positioned(p.map(WithPos.apply)).flatMap { positionedValue =>
      positionedValue.pos match {
        case pos: OffsetPosition =>
          ...
          if (pre.trim.isEmpty)
            failure(
              s"`${positionedValue.value}` should not appear immediately after newline"
            )
...
//    parsedTsFile(content)
    ...
    (in: Reader[lexical.Token]) =>
      f(in) match {
        case Failure(_, next: Input)
            if next.first.isInstanceOf[lexical.CommentToken] || next.first
              .isInstanceOf[Directive] =>
          f(next.rest)
        case fail: Failure =>
          fail
        case other =>
  ...
  val operator: Parser[String] = {
    ...
    val other = accept(
      "Operator",
      { case lexical.Keyword(chars) if ops contains chars => chars }
    )
  ...
  lazy val directive: Parser[Directive] =
    accept(
      "Directive",
      {
        case lexical.DirectiveToken("reference", "lib", value) =>
          Directive.LibRef(value)
        case lexical.DirectiveToken("reference", "types", value) =>
          Directive.TypesRef(value)
        case lexical.DirectiveToken("references", "types", value) =>
          Directive.TypesRef(
            value
          ) // not sure if it's a typo, but we'll retain the hint
        case lexical.DirectiveToken("reference", "path", value) =>
          Directive.PathRef(value)
        case lexical.DirectiveToken("reference", "no-default-lib", "true") =>
          Directive.NoStdLib
        case lexical.DirectiveToken("amd-module", "name", value) =>
          Directive.AmdModule(value)
      }
    ).named("directive")
  ...
  lazy val parsedTsFile: Parser[TsParsedFile] =
    shebang.? ~> comments ~ directives ~ tsContainerOrDecls ~ success(
      CodePath.NoPath
    ) ^^ TsParsedFile.apply
  ...
  lazy val tsDeclNamespace: Parser[TsDeclNamespace] =
    comments ~ (isDeclared <~ "namespace") ~ rep1sep(
      tsIdent,
      "."
    ) ~ tsContainerOrDeclBody ^^ {
  ...
  lazy val tsDeclModule: Parser[TsNamedDecl] = {
    comments ~ (isDeclared <~ "module") ~ either(
      tsIdentModule,
      rep1sep(tsIdent, ".")
    ) ~ tsContainerOrDeclBody.? ^^ { case cs ~ declared ~ nameEither ~ body =>
      nameEither match {
        case Left(name) =>
          TsDeclModule(
            cs,
  ...
  lazy val tsGlobal: Parser[TsGlobal] =
    comments ~ (isDeclared <~ "global") ~ tsContainerOrDeclBody ~ success(
      CodePath.NoPath
    ) ^^ TsGlobal
  ...
  lazy val tsImport: Parser[TsImport] = {
    ...
    val raw: Parser[TsImport] = {
      ...
      "import" ~> moduleIdentMaybeParams ^^ (id =>
        TsImport(
          typeOnly = false,
  ...
  val tsExport: Parser[TsExport] = {
    ...
    (comments <~ "export") ~ exportType ~! exportee ^^ {
      case cs ~ exportType ~ (typeOnly ~ exportee) =>
        TsExport(cs, typeOnly, exportType, exportee)
    }
  }
  ...
  lazy val tsDeclVar: Parser[TsDeclVar] = tsDeclVars ^^ {
    ...
    case IArray.headTail(first, rest) =>
      first.copy(comments =
        Comments(Comment.warning(s"Dropped ${rest.map(_.name.value)}"))
      )
  }
  ...
  lazy val tsDeclVars: Parser[IArray[TsDeclVar]] = {
    ...
    comments ~ isDeclared ~ (variable | constant) ~ rep1sep_(
      tsIdent ~ typeAnnotationOpt ~ ("=" ~> expr).?,
      ","
    ) ^^ { case cs ~ declared ~ isReadonly ~ vars =>
      vars.map { case name ~ tpe ~ expr =>
        TsDeclVar(
          cs,
          declared,
          isReadonly,
  ...
  lazy val tsDeclEnum: Parser[TsDeclEnum] =
    comments ~ isDeclared ~ ("const".isDefined <~ "enum") ~ tsIdent ~ ("{" ~> tsEnumMembers <~ "}") ~ success(
      true
    ) ~ success(
      None
  ...
  lazy val tsDeclClass: Parser[TsDeclClass] = {
    ...
    val functionCall: Parser[TsTypeRef] =
      expr.filter(!_.isInstanceOf[TsExpr.Ref]) ^^ { notRef =>
        ...
        TsTypeRef.any.copy(
          Comments(Comment.warning(s"class extends from : $str"))
        )
      }
    ...
    /** hack to avoid that `identifier` consumes `extends` for default exported
  ...
  lazy val typeParam: Parser[TsTypeParam] =
    comments ~ tsIdent ~ ("extends" ~>! perhapsParens(
      tsType
    )).? ~ ("=" ~>! tsType).? ^^ TsTypeParam.apply

  lazy val tsTypeParams: Parser[IArray[TsTypeParam]] =
    "<" ~>! repsep_(typeParam, ",") <~! ",".? <~ comments.? <~! ">" | success(
      Empty
    )
  ...
  val tsFunctionParams: Parser[IArray[TsFunParam]] = functionParam.** ^^ {
    params =>
      params.zipWithIndex.map {
        case (p @ TsFunParam(_, TsIdent.Destructured, _), i) =>
          p.copy(name = TsIdent(s"param$i"))
        case (p, _) => p
      }
  }
  ...
  lazy val functionParam: Parser[TsFunParam] = {
    ...
    /** Note: we don't care about the specifics of a destructured parameter. we
      * just want a unique name and a type *
      */
    lazy val destructuredObj: Parser[TsIdentSimple] =
      "{" ~>! rep(
        (tsIdentLiberal | ("..." ~> tsIdent)) ~ (":" ~> (tsIdent | destructured)).? <~ ",".?
      ) <~ "}" ^^ (_ => TsIdent.Destructured)
    lazy val destructuredArray: Parser[TsIdentSimple] =
      "[" ~>! ",".? ~> repsep(
        "...".? ~> tsIdent <~ (":" <~ (tsIdent | destructured)).?,
        ","
      ) <~ "]" ^^ (_ => TsIdent.Destructured)
    ...
    comments ~ "...".isDefined ~ (tsIdent | destructured) ~ "?".isDefined ~ (typeAnnotationOpt <~ ("=" ~ expr).?) <~ ",".? ^^ {
      ...
      case cs ~ _ ~ i ~ o ~ t =>
        val warning =
          s"Dropping repeated marker of param ${i.value} because its type ${t
              .fold("<none>")(tpe => TsTypeFormatter(tpe))} is not an array type"
        TsFunParam(
          cs + Comment.warning(warning),
          i,
          t.map(OptionalType.maybe(_, o))
        )
    }
  }
  ...
  lazy val expr: Parser[TsExpr] = {
    ...
    val call: Parser[TsExpr.Call] =
      ref ~ ("(" ~> upgrade(repsep(expr, ",")) <~ ",".? <~ ")") ^^ TsExpr.Call
    ...
    lazy val binaryOp: Parser[TsExpr.BinaryOp] = {
      val first =
        ("(" ~> binaryOp <~ ")") | base /* avoid left recursion for ever */
  ...
  lazy val baseTypeDesc: Parser[TsType] = {
    val tsTypeFunction: Parser[TsTypeFunction] =
      comments ~ tsTypeParams ~ ("(" ~> tsFunctionParams <~ ")") ~ ("=>" ~> tsType) ^^ {
        case cs ~ tparams ~ params ~ resultType =>
          TsTypeFunction(TsFunSig(cs, tparams, params, Some(resultType)))
      }
    ...
    ((tsIdent <~ "is") ~ tsType ^^ TsTypeIs
      ...
      | "unique" ~> "symbol" ~> success(
        TsTypeRef(NoComments, TsQIdent.symbol, Empty)
      )
      | "typeof" ~> tsTypeRef ^^ { case TsTypeRef(_, name, _) =>
        TsTypeQuery(name)
      } // todo: targs may be used to with `typoeof f<asd>`
      | tsTypeTuple
      | "(" ~> tsType <~ ")"
      | tsLiteral ^^ TsTypeLiteral
      | tsLiteralTemplateString ^^ (chars =>
        TsTypeRef.string
          .copy(comments = Comments(s"/* template literal string: $chars */")),
      )
  ...
  lazy val tsType: Parser[TsType] = {
    ...
    val _extends =
      union ~ "extends" ~ tsType ^^ { case _1 ~ _ ~ _2 =>
        TsTypeExtends(_1, _2)
      }

    lazy val conditional: Parser[TsType] =
      _extends ~ "?" ~ tsType ~ ":" ~ tsType ^^ { case _1 ~ _ ~ _2 ~ _ ~ _3 =>
        TsTypeConditional(_1, _2, _3)
      }
  ...
  lazy val tsTypeTuple: Parser[TsTypeTuple] = {
    val repeatedElem: Parser[TsTupleElement] =
      "..." ~>! (tsIdent <~ ":").? ~ tsType ^^ { case label ~ tpe =>
        TsTupleElement(label, TsTypeRepeated(tpe))
      }
    ...
    val nonRepeatedElem: Parser[TsTupleElement] =
      (tsIdent ~ "?".isDefined <~ ":").? ~ unlabeledNonRepeatedElem ^^ {
        case None ~ tpe                => TsTupleElement(None, tpe)
        case Some(label ~ false) ~ tpe => TsTupleElement(Some(label), tpe)
        case Some(label ~ true) ~ tpe =>
          TsTupleElement(
            Some(label),
            TsTypeUnion(IArray(tpe, TsTypeRef.undefined))
          )
      }
  ...
  lazy val tsTypeRef: Parser[TsTypeRef] = {
    val typeArgs: Parser[IArray[TsType]] =
      (notAfterNewline("<") ~> rep1_(tsType <~ ",".?) <~ ">") | success(
        IArray.Empty
      )
  ...
  lazy val tsMemberNamed: Parser[TsMember] = {
    ...
    val intro: Parser[
      (TsProtectionLevel, TsIdentSimple, Boolean, Boolean, MethodType)
    ] =
      tsIdentLiberal.+ ^^ { case mods :+ name =>
        val level: TsProtectionLevel =
          if (mods.contains(TsIdent("protected"))) TsProtectionLevel.Protected
          else if (
            mods.contains(TsIdent("private")) || name.value.startsWith("#")
      ...
      comments ~ intro ~ "?".isDefined ~ functionSignature ^^ {
        ...
        case cs ~ ((
          ...
          TsMemberProperty(
            ...
            level,
            name,
            Some(OptionalType(TsTypeFunction(sig))),
            None,
            static,
            readOnly
          )
      ...
      comments ~ intro ~ "?".isDefined ~ typeAnnotationOpt ~ ("=" ~> expr).? ^^ {
        case cs ~ ((
          ...
          TsMemberProperty(
            ...
            level,
            name,
            tpe.map(OptionalType.maybe(_, optional)),
            expr,
            static,
            readonly
          )
      ...
      comments ~ "readonly".isDefined ~ (protectionLevel <~ "readonly".isDefined) ~ indexing ~ "?".isDefined ~ indexedType.? ~ ("=" ~> expr).? ^^ {
        case cs ~ readOnly ~ level ~ indexing ~ isOptional ~ valueType ~ exprOpt =>
          TsMemberIndex(
            ...
            indexing,
            valueType
              .map(OptionalType.maybe(_, isOptional))
              .orElse(exprOpt.map(TsExpr.typeOf))
          )
      }
  ...
  lazy val tsIdentLiberal: Parser[TsIdentSimple] =
    tsIdent | ((tsLiteralString | "[" ~> tsLiteralString <~ "]") ^^ (lit =>
      TsIdent(lit.value)
    ))
  ...
  lazy val qualifiedIdent: Parser[TsQIdent] = {
    ...
    val withImport = tsIdentImport ~ rep_("." ~> tsIdent) ^^ { case i ~ rest =>
      TsQIdent(i +: rest)
    }
    withImport | normal
  }
  ...
  object ArrayType {
    ...
    def unapply(typeRef: TsTypeRef): Option[TsType] =
      typeRef match {
        case TsTypeRef(NoComments, TsQIdent.Array, IArray.exactlyOne(elem)) =>
          Some(elem)
        case TsTypeRef(
              NoComments,
              TsQIdent.ReadonlyArray,
